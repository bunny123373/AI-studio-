...
export async function POST(req: Request) {
  ...
  // Engine override from the tool's "Image engine" picker (gemini | pollinations).
  const requested = typeof p.provider === "string" ? p.provider.trim() : "";
  const providerId = ["gemini", "pollinations", "local", "none"].includes(
    requested,
  )
    ? requested
    : env.imageProvider;
  ...
  if (!provider.configured) {
    return json({
      ok: false,
      error:
        "No image provider is configured. Set IMAGE_PROVIDER=gemini (needs GEMINI_API_KEY + image quota) or IMAGE_PROVIDER=pollinations (free, keyless), then restart. See .env.example.",
    });
  }
  let result = await provider.generate(input);
  let notice: string | undefined;

  // Honest fallback: Gemini image models are billed separately from text, and
  // this account currently has no image quota (429 RESOURCE_EXHAUSTED). Instead
  // of dead-ending the tool, fall back to free Pollinations and say so clearly.
  if (!result.ok && provider.id === "gemini") {
    const fb = getImageProviderFor("pollinations");
    if (fb.configured) {
      const fbResult = await fb.generate(input);
      if (fbResult.ok) {
        notice =
          "Gemini image generation has no quota on this account — this image was generated by the free Pollinations service instead. Enable image billing on your Gemini key to use it directly.";
        result = fbResult;
      } else {
        result = {
          ...result,
