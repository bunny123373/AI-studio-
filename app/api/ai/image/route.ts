...
export async function POST(req: Request) {
  ...
  const ENGINE_NAMES: Record<string, string> = {
    ...
    none: "disabled",
    openrouter: "OpenRouter (Ling VL)",
  };

  // OpenRouter / Ling 3.0 Flash VL is a LANGUAGE-VISION model — it cannot
  // generate images. When picked as the engine, it refines the prompt and
  // the free Pollinations service paints the picture; the label says so.
  if (providerId === "openrouter") {
      ...
      return json(
        {
          ...
          provider: "openrouter",
          error:
            "OPENROUTER_API_KEY is not set. OpenRouter (Ling 3.0 Flash VL) only refines the prompt — it cannot paint images. Set the key, or pick Gemini, Hugging Face or Pollinations.",
        },
        400,
      );
    }
    let refinedPrompt = styledPrompt;
    let refinedBy = "";
    try {
      const expanded = await openrouterProvider.generate(styledPrompt, {
        system:
          "You are an expert text-to-image prompt writer. Rewrite the user's idea into ONE detailed prompt (subject, setting, lighting, camera, mood, colours). English only. No markdown, no quotes, no labels — just the prompt.",
      });
      const cleaned = expanded.slice(0, 1500).trim();
      if (cleaned) {
        refinedPrompt = cleaned;
        refinedBy = " OpenRouter (Ling 3.0 Flash VL) refined the prompt.";
      }
    } catch {
      // Keep the original prompt; Pollinations still paints it.
    }
    const painted = await getImageProviderFor("pollinations").generate({
      ...input,
    ...
    if (painted.ok) {
      return json({
        ...
        notice: `Ling 3.0 Flash VL is a vision-language model, so it cannot paint images.${
          refinedBy
            ? refinedBy
            : " OpenRouter was not reachable, so your original prompt was used."
        } The image was generated by the free Pollinations engine.`,
      });
    }
    return json({
      ok: false,
      provider: "openrouter",
      error: `Prompt ready, but the free Pollinations engine failed to paint it: ${painted.error}`,
    });
  }
